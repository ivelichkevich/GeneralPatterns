...
public class NullCheckingTestGenerator implements TestGenerator {
        ...
        for (ClassOrInterfaceDeclaration classDeclaration : classes) {
            ...
            for(MethodDeclaration methodDeclaration : classDeclaration.getMethods()) {
                if (methodDeclaration.getParameters().isEmpty() || !methodDeclaration.getBody().isPresent()
                        || !methodDeclaration.getBody().get().toString().contains(PATTERTN_2)) {
                    continue;
                }
                NodeList<Parameter> parameters = methodDeclaration.getParameters();
                for (MethodCallExpr call : findMethodsCall(methodDeclaration, PATTERTN_2)) {
    ...
    private Optional<DependableNode<MethodDeclaration>> buildMethodTest(
        ...
        Optional<DependableNode<MethodCallExpr>> maybeMethod = invocationBuilder.usingForParameters(expressionsForParameters).buildMethodInvocation(methodDeclaration);
...
//    public static List<MethodCallExpr> findMethodsCall(Node node, String methodName) {
//        return node.findAll(MethodReferenceExpr.class, n -> n.getNameAsString().equals(methodName));
//    }
}